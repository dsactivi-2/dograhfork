...
def stt_uses_external_turns(user_config) -> bool:
    ...
    if (
        user_config.stt.provider == ServiceProviders.DEEPGRAM_EU.value
    ):  # CUSTOM-SEAM: deepgram_eu
        return deepgram_eu_uses_external_turns(user_config)
    if (
        user_config.stt.provider == ServiceProviders.DEEPGRAM_2.value
    ):  # CUSTOM-SEAM: deepgram_2
        return deepgram_2_uses_external_turns(user_config)
    if (
        user_config.stt.provider == ServiceProviders.DEEPGRAM_3.value
    ):  # CUSTOM-SEAM: deepgram_3
...
def create_tts_service(
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
        ...
        user_config.tts.provider == ServiceProviders.DEEPGRAM_EU.value
    ):  # CUSTOM-SEAM: deepgram_eu
        return create_deepgram_eu_tts(user_config, audio_config)
    elif (
        user_config.tts.provider == ServiceProviders.FISH_AUDIO.value
    ):  # CUSTOM-SEAM: fish_audio
