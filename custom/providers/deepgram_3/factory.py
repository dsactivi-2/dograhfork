...
def create_deepgram_3_stt(user_config, audio_config, keyterms=None):
    ...
    if is_flux_model(model):
        settings_kwargs = {
            ...
            "keyterm": (keyterms or [])
            if live_flag(user_config.stt, "keyterm_prompting")
            else [],
        }
    ...
    settings_kwargs = live_stt_settings_kwargs(
        user_config,
        model=model,
        language=language,
        keyterms=keyterms,
    )
    settings_kwargs["numerals"] = getattr(
        user_config.stt, "numerals", STT_DEFAULTS["numerals"]
    )
    extra = dict(settings_kwargs.get("extra") or {})
    extra["vad_events"] = getattr(
        user_config.stt, "vad_events", STT_DEFAULTS["vad_events"]
    )
